...
class RandomGenerators:
  ...
  def __init__(self,seed=None,carry=None):
    #lcg
    self.lcg_mod=2**31 #10
    self.lcg_a=1103515245#2
    self.lcg_c=12345#3
    #bbs
  ...
  def next_LCG(self):
    ...
    self.lcg_prev=rand_num
    return self.lcg_prev/float(self.lcg_mod) # between 0 and 1
  ...
  def next_BlumBlumShub(self):
    ...
    return rand_num/float(self.bbs_mod) # between 0 and 1
  ...
  def next_xorshift(self):
    ...
    return (rand_num & 0xFFFFFFFF)/float(0xFFFFFFFF) #(& 0xFFFFFFFF trunctates the number), divide by uint32 value for [0,1)
